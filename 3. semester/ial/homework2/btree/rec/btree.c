...
bool bst_search(bst_node_t *tree, char key, int *value) {
    ...
    if (key < tree->key) {                          // Hledáme klič v levém podstromu
        return bst_search(tree->left, key, value);
    } else if (key > tree->key) {                   // Hledáme klič v pravém podstromu
...
void bst_dispose(bst_node_t **tree) {
    ...
    // Rekurzivní zrušení levého a pravého podstromu
    bst_dispose(&(*tree)->left);
    bst_dispose(&(*tree)->right);

    // Uvolnění a inicializace uzlu
    free(*tree);
    bst_init(tree);
}
